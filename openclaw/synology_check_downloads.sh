#!/usr/bin/env bash
# Synology DownloadStation API health check - runs independently
set -euo pipefail
...
call_json() {
  ...
  if jq -e '.isError == true' >/dev/null <<<"$out"; then
    ...
    # Known expected error: get_list_of_tasks API mismatch
    if [[ "$msg" == *"get_list_of_tasks"* ]]; then
      warn "$check" "downloadstation_api_mismatch"
    else
      crit "$check" "$msg"
    fi
  fi
  ok "$check" "downloadstation_api_healthy"
}

check_downloads() {
  local check="downloadstation"
  call_json "$check" "synology.synology_list_downloads" '{"params":{"offset":0,"limit":10}}'
}
