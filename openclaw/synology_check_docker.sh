#!/usr/bin/env bash
# Synology Docker API health check - runs independently
set -euo pipefail
...
call_json() {
  ...
  if jq -e '.isError == true' >/dev/null <<<"$out"; then
    ...
    # Known expected error: SYNO.Docker.Container API not available
    if [[ "$msg" == *"SYNO.Docker.Container"* ]]; then
      warn "$check" "docker_api_missing"
    else
      crit "$check" "$msg"
    fi
  fi
  ok "$check" "docker_api_healthy"
}

check_docker() {
  local check="docker"
  call_json "$check" "synology.synology_docker_list_containers" '{"params":{}}'
}
