#!/usr/bin/env bash
# Synology HyperBackup API health check - runs independently
set -euo pipefail
...
call_json() {
  ...
  if jq -e '.isError == true' >/dev/null <<<"$out"; then
    ...
    # Known expected error: SYNO.Backup.Task API not available
    if [[ "$msg" == *"SYNO.Backup.Task"* ]]; then
      warn "$check" "hyperbackup_api_missing"
    else
      crit "$check" "$msg"
    fi
  fi
  ok "$check" "hyperbackup_api_healthy"
}

check_backup() {
  local check="backup"
  call_json "$check" "synology.synology_backup_list" '{"params":{}}'
}
